...
const server = net.createServer((socket) => {
    ...
    const welcomeMessage = new Chat(_id, 'Welcome !', Chat.INFO_TYPE.alarm, true);
    socket.write(JSON.stringify(welcomeMessage));
    console.log('클라이언트 연결 됨', socket.remoteAddress, ':', socket.remotePort, '/ id: ', _id);

    // 다른 유저들에게 새 유저 입장 알림 후 클라이언트 목록에 새로 추가.
    clients.forEach(c => {
        const entranceAlarm = new Chat(_id, `${_id}님이 대화방에 입장하셨습니다`, Chat.INFO_TYPE.alarm, false);
